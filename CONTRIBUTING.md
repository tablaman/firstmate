...
If the check fails because the attestation `head_sha` no longer matches the PR head - which happens whenever a commit reaches the PR branch without going through the gate - recover by re-running `git push no-mistakes` from that branch so the pipeline rebinds the PR body attestation to the new head.
